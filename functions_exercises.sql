USE employees;
# Modify your first query to order by first name. The first result should be Irena Reutenauer
...
# Change the order by clause so that you order by last name before first name. Your first result
...
SELECT datediff(currdate(), hire_date)
FROM employees
